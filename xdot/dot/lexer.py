...
class Lexer:
  ...
  def __init__(self, buf: Optional[bytes] = None, pos: int = 0,
      filename: None = None, fp: None = None) -> None:
    if fp is not None:
      try:
        fileno = fp.fileno()
  ...
  def consume(self, text: bytes) -> None:
    # update line number
    pos = 0
...
class DotLexer(Lexer):
  ...
  def filter(self, type: int, text: bytes) -> Tuple[int, bytes]:
    # TODO: handle charset
    if type == STR_ID:
      text = text[1:-1]
